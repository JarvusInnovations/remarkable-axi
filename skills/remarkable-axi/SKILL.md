---
name: remarkable-axi
description: Send documents to a reMarkable tablet, design pages for its e-ink panel, and pull handwriting back off it via the remarkable-axi CLI. Use when sending a PDF/EPUB/URL to a tablet, designing or checking a page against the device's screen, or pulling ink, annotations, or typed text back down. Also use the moment handwriting or annotations look lost — a reMarkable showing blank pages that were written on, ink missing after a sync, or "did my notes survive that replace" — even before you know this tool is the fix; it carries the ink-recovery playbook.
metadata:
  hermes:
