...
`put --replace`, `put --keep-ink`, and any future operation that swaps a document's
contents.
...
`--keep-ink` ports the superseded document's strokes onto the new one. Ink is stored
per page and positioned in page-relative coordinates, so where the page box is
